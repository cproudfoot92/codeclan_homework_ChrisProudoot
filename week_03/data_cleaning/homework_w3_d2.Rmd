...
```{r}
face <- read_csv("data/face_descriptions.csv")
...
-The variable names are not suitable as they are in wide format.
...
```{r}
face_longer <- pivot_longer(face, cols = starts_with("t"),
             names_to = "faces",
             values_to = "characteristcs")
face_longer
```
...
```{r}
face_separated <- face_longer %>%
  separate(characteristcs, 
           into = c("characteristc1","characteristc2","characteristc3" ), 
           sep = "([,./?])")
...
```{r}
all_description <- face_separated %>%
  pivot_longer(cols = starts_with("characteristc"),
               names_to = "description_number",
               names_prefix = "characteristc",
...
```{r}
filtered_description <- all_description %>%
...
```{r}
grouped_faces <- filtered_description %>%
